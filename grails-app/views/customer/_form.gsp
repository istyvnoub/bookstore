<div class="form-group">
  ...
  <UI:renderErrormessage fieldName="firstName" model="${customer}" errorMessage="please.enter.first.name"/>
</div>
...
<div class="form-group">
  ...
  <UI:renderErrorMessage fieldName="email" model="${customer}" errorMessage="Your Email Address is not Valid / Already Exist in System"/>
</div>
...
<g:if test="${!edit}">%{-- if edit=yess then Passwordfiel will not appear--}%
  <div class="form-group">
    ...
    <UI:renderErrorMessage fieldName="password" model="${customer}" errorMessage="Please Enter a Password."/>
  </div>
</g:if>
